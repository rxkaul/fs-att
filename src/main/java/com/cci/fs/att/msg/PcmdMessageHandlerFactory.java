...
/**
 * A factory for creating PcmdMessageHandler objects.
 * 
 * @author CCI
 * @version $Rev$
 */
@Component
public class PcmdMessageHandlerFactory implements MessageHandlerFactory {
	...
	public void setEnodeBCoords(String coords) {
		...
		version = Integer.parseInt(coords.substring(0, pos));
	...
	public List<MessageHandler> createMessageHandlers() {
		for (int port : ports) {
			PcmdMessageHandler handler = new PcmdMessageHandler(port, zkHost, version,
					eNodebCoords, enableCache);
